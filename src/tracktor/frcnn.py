...
class FRCNN(resnetv1):
    ...
    def forward(self, rois):
        # 在当前图中使用前面帧的roi坐标回归
        # rois是前面帧的轨迹
        net_conv = self._net_conv # 当前帧
        # #对产生的porposal进行ROI池化，统一格式
        ...
        # self._predictions来自resnet_v1类里继承的父类Network

        self._predictions["fc7"] = fc7
        # 进行分类和预测框回归，bbox_pred是回归的结果
        ...
        # 此处的rois的坐标值和传入的一样，没变，只是去掉了前面的第一列sc值？
