...
class Tracker():
	...
	def __init__(self, obj_detect, tracker_cfg):
		self.obj_detect = obj_detect  # 目标检测器
	...
	def add(self, new_det_pos, new_det_scores):
		...
		num_new = new_det_pos.size(0) # 新开辟的轨迹数目
		# (self, pos, score, track_id, features, inactive_patience, max_features_num)
		# inactive_patience = 10 表示被kill的轨迹的保存时间帧
		# max_features_num = 10 保存的特征向量数量
	...
	def regress_tracks(self, blob):
		...
		pos = self.get_pos() # 此处取出的是之前轨迹的pos坐标

		# regress
		# 得到前面所有帧轨迹的ROI在当前帧中的分类预测
		# bbox_pred是默认框的偏移量（回归量dx,dy,dw,dh
		...
		for i in range(len(self.tracks)-1,-1,-1):
			t = self.tracks[i]
			t.score = scores[i] # 将此轨迹的score更新为当前帧的得分。
			# regression_person_thresh = 0.5
			if scores[i] <= self.regression_person_thresh: #　小于阈值的将被kill,存入未被激活
				self.tracks_to_inactive([t])
			else:
				s.append(scores[i])
				# t.prev_pos = t.pos
				t.pos = pos[i].view(1,-1)  # 更新每条轨迹新的坐标值
	...
	def align(self, blob):
		...
		if self.im_index > 0:   # 初始是０
			im1 = self.last_image.cpu().numpy()		# last image
			im2 = blob['data'][0][0].cpu().numpy()	# current image
			im1_gray = cv2.cvtColor(im1,cv2.COLOR_BGR2GRAY)
			im2_gray = cv2.cvtColor(im2,cv2.COLOR_BGR2GRAY)
			sz = im1.shape
			warp_mode = self.warp_mode # 欧氏运动模型：平移＋旋转
			# 输出对角阵，两行三列，对角线元素为１，其余为０
			warp_matrix = np.eye(2, 3, dtype=np.float32)
			#number_of_iterations = 5000
			number_of_iterations = self.number_of_iterations # 100
	...
	def step(self, blob):
		...
		if rois.nelement() > 0:  # 元素个数
			boxes = bbox_transform_inv(rois, bbox_pred)
			boxes = clip_boxes(Variable(boxes), blob['im_info'][0][:2]).data

			# Filter out tracks that have too low person score
			# self.cl = 1 表示只取行人类别的数据
			scores = scores[:, self.cl]
			# 逐元素比较input和other ， 即是否input>other 如果两个张量有相同的形状和元素值，则返回True ，否则 False。
			# 此处用于过滤小于阈值的检测框
		...
		if len(self.tracks):
			...
			if len(self.tracks):
				...
				if keep.nelement() > 0:
					...
					# new_features = self.get_appearances(blob) # reid
		...
		if det_pos.nelement() > 0: # 当前帧所有的检测框
		...
		for t in self.tracks:       # 每一帧都更新results
			track_ind = int(t.id)
			if track_ind not in self.results.keys(): # 添加新的轨迹，以轨迹ID作为KEY
