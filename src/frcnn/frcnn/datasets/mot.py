...
class mot(imdb):
    ...
    def _load_mot_annotation(self, index):
        ...
        gt_file = os.path.join(os.path.dirname(os.path.dirname(image_pth)), 'gt', 'gt.txt')
    ...
    def _python_eval(self, all_boxes, ovthresh=0.5):
        ...
        tp = [[[] for _ in range(len(self.image_index))] for _ in range(self.num_classes)]
        fp = [[[] for _ in range(len(self.image_index))] for _ in range(self.num_classes)]
        ...
        for cls_index, cls in enumerate(self._classes):
            ...
            for im_gt in gt_roidb:
                bbox = im_gt['boxes'][np.where(im_gt['gt_classes'] == cls_index)]
